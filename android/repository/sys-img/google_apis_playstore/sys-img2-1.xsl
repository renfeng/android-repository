...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/*">
		<xsl:for-each select="*[not(@obsolete) or @obsolete!='true']/archives/archive/complete/url">
			...
			<xsl:choose>
				<xsl:when test="starts-with($url, 'https://dl.google.com/android/repository/sys-img/google_apis_playstore/')">
				...
				<xsl:otherwise>
					<xsl:text>https://dl.google.com/android/repository/sys-img/google_apis_playstore/</xsl:text>
					<xsl:value-of select="$url"/>
				</xsl:otherwise>
			</xsl:choose>
